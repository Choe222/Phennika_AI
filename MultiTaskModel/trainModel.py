...
#With the small dataset, we will not split into train and validation sets
"""
VALID_SIZE = 0.2
RANDOM_STATE = 42
...
def evaluate(model, dataloader, device):
    ...
    binary_relevance_preds = (all_relevance_preds > 0.7).int()
...
def train_and_evaluate(model, train_loader, val_loader, epochs=10, learning_rate=3e-5, alpha=0):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        if overall_score > best_overall_score:
            ...
            torch.save(model.state_dict(), 'Your_parameter_file')
...
if __name__ == "__main__":
    ...
    model.to(device)
    try:
        state_dict = torch.load('Your_parameter_file', map_location=device)
        model.load_state_dict(state_dict)
        print("Loading model from checkpoint successfully.")
    except (FileNotFoundError) as e:
        print(f"Error when loading checkpoint: {e}. Training from scratch...")
    trained_model = train_and_evaluate(model, train_loader, train_loader, epochs=10, learning_rate=1e-5, alpha = 0.3)